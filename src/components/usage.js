...
function render(panel, d) {
  ...
  body.innerHTML = `
    ...
    <div class="usage-section-head">
      ...
      <span class="usage-toggle" data-metric>${metric === 'cost' ? '$ → tokens' : 'tokens → $'}</span>
    </div>
...
export function initUsage() {
  ...
  panel.innerHTML = `
    <div class="usage-head">
      ...
      <span class="usage-actions"><span class="usage-refresh" title="Refresh">⟳</span><span class="usage-close" title="Close">✕</span></span>
    </div>
  ...
  style.textContent = `
    #usage-btn {
      ...
      box-shadow: 0 0 16px rgba(var(--accent-rgb,41 211 255),.22);
    }
    ...
    #usage-panel {
      ...
      border: 1px solid rgba(var(--accent-rgb,41 211 255),.42);
      border-radius: 12px;
    ...
    .usage-head { display:flex; justify-content:space-between; align-items:center; padding:11px 14px;
      border-bottom:1px solid rgba(var(--accent-rgb,41 211 255),.25); color:rgb(var(--accent-rgb,41 211 255));
      font-size:12px; letter-spacing:1.5px; font-weight:600; }
    .usage-actions span { cursor:pointer; margin-left:10px; opacity:.8; }
    ...
    .usage-kpi { background:rgba(var(--accent-rgb,41 211 255),.07); border:1px solid rgba(var(--accent-rgb,41 211 255),.18);
      border-radius:9px; padding:9px 10px; }
    .usage-kpi-v { font-size:19px; font-weight:700; color:rgb(var(--accent-rgb,41 211 255)); }
    .usage-kpi-l { font-size:9px; letter-spacing:1px; opacity:.6; margin-top:2px; }
    .usage-section-head { display:flex; justify-content:space-between; align-items:center; font-size:9.5px; letter-spacing:1.5px; opacity:.65; margin:10px 0 6px; }
    .usage-toggle { cursor:pointer; color:rgb(var(--accent-rgb,41 211 255)); opacity:.9; }
    ...
    .usage-bar-track { flex:1; height:7px; background:rgba(255,255,255,.06); border-radius:4px; overflow:hidden; }
    .usage-bar-fill { display:block; height:100%; background:rgb(var(--accent-rgb,41 211 255)); border-radius:4px; }
    .usage-bar-val { width:54px; text-align:right; color:rgb(var(--accent-rgb,41 211 255)); font-variant-numeric:tabular-nums; }
    ...
    .usage-events { border:1px solid rgba(var(--accent-rgb,41 211 255),.16); border-radius:10px; overflow:hidden; margin-bottom:10px; }
    .usage-event-row { display:grid; grid-template-columns:1.1fr 1fr auto; gap:6px; padding:8px 9px; border-bottom:1px solid rgba(255,255,255,.06); font-size:10px; }
    .usage-event-row:last-child { border-bottom:0; }
    .usage-event-type { color:rgb(var(--accent-rgb,41 211 255)); font-weight:700; letter-spacing:.8px; text-transform:uppercase; }
    ...
    .usage-event-cost { color:rgb(var(--accent-rgb,41 211 255)); font-variant-numeric:tabular-nums; font-weight:700; text-align:right; }
  ...
  panel.querySelector('.usage-close').addEventListener('click', () => panel.classList.remove('open'));
