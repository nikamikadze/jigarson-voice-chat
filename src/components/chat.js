...
async function speakText(text) {
  dbg('speakText.called', { enabled: ttsEnabled, len: text?.length || 0 });
  if (!ttsEnabled || !text) return;
  try {
    // 限制長度，避免 TTS 卡太久（超過 500 字截斷）
    ...
    dbg('speakText.fetch', { status: res.status, type: res.headers.get('content-type') });
    if (!res.ok) return;

    const blob = await res.blob();
    dbg('speakText.blob', { size: blob.size, type: blob.type });
    ...
    dbg('speakText.exception', { err: String(e).slice(0, 140) });
  }
}
...
function handleChatEvent(data) {
  ...
  if (data.activity) {
    window.dispatchEvent(new CustomEvent('openclaw-activity', {
  ...
  if (done) {
    ...
    if (replyBuffer) {
      ...
      // 語音模式下由 /api/voice 負責 TTS，避免同一句講兩次互相打斷
      if (isVoiceActive()) {
        dbg('speakText.skipped-voice-mode');
      } else {
...
async function handleChatSend() {
  ...
  window.dispatchEvent(new CustomEvent('openclaw-activity', {
    detail: { kind: 'state', text: hasFiles ? 'uploading files to OpenClaw' : 'sending message to OpenClaw' },
  }));
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('openclaw-activity', {
      detail: { kind: 'state', text: activeRunId ? `run accepted (${activeRunId})` : 'run accepted' },
    }));
