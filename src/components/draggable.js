...
function loadPanelPositions() {
  try {
    ...
    Object.entries(data).forEach(([sel, pos]) => {
      const el = document.querySelector(sel);
      if (!el) return;
      // 還原位置（並夾住在畫面內，避免拖到看不見）
      if (pos.x != null && typeof gsap !== 'undefined') {
        let x = pos.x, y = pos.y;
        gsap.set(el, { x, y });
        let d = Draggable.get(el);
        if (d) d.update();
        // Clamp back into the viewport if the saved spot is off-screen.
        const r = el.getBoundingClientRect();
        const m = 10;
        if (r.top < 0) y += -r.top + m;
        if (r.left < 0) x += -r.left + m;
        if (r.right > window.innerWidth) x += window.innerWidth - r.right - m;
        if (r.bottom > window.innerHeight) y += window.innerHeight - r.bottom - m;
        if (x !== pos.x || y !== pos.y) {
          gsap.set(el, { x, y });
          if (d) d.update();
        }
