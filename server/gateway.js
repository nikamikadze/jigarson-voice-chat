...
let gatewayUrl = '';
let gatewayToken = '';
let configSessionKey = null;  // config 定義的 session key (main / chat panel)
let extraSessionKeys = new Set();  // additional session keys to accept (e.g. voice agent)
...
export function initGateway({ url, token, sessionKey, onChat, extraSessionKeys: extras }) {
  gatewayUrl = url;
  gatewayToken = token;
  configSessionKey = sessionKey;
  extraSessionKeys = new Set((extras || []).filter(Boolean).filter(k => k !== sessionKey));
  onChatEvent = onChat;
  connect();
}
...
function sendConnect() {
  ...
  const auth = authToken ? { token: authToken, password: authToken } : undefined;
...
function connect() {
  ...
  gw.on('message', (raw) => {
    ...
    try {
      ...
      console.error('[GW] dropped non-JSON frame:', err.message);
      return;
    }
    try {
      handleGatewayMessage(msg);
    } catch (err) {
      console.error('[GW] message handler error:', err.message);
    }
  });
  ...
  function handleGatewayMessage(msg) {
    ...
    if (msg.type === 'event' && msg.event === 'chat') {
      ...
      if ((sk === configSessionKey || extraSessionKeys.has(sk)) && onChatEvent) {
        const p = msg.payload;
